...
本模块协议功能基于 websocket 实现，使用 json 格式数据进行数据交互。

本模块设计 `ocpp.charge_point.ChargePoint` 抽象类，用来做为基类，用户需要根据自己项目的实际情况进行二次开发，基于该抽象类封装自己项目需要用到的，对应版本的，发送和接收功能。

目前项目只支持 OCPP v1.6 版本的请求和接收的消息数据解析，后续会继续进行扩展支持。

本文主要分为两个大类进行功能说明：

1. 基于 `ChargePoint` 基类实现数据发送和接收接口的二次开发
2. OCPP v1.6 版本的客户端发送消息数据和接收消息数据，枚举值说明
...
- 该抽象类封装了基于 websocket 协议的数据发送 `call` 接口和用于接收服务器消息并处理的 `start` 接口。
- 该抽象类的二次开发，需要配合 `ocpp.routing.on` 和 `ocpp.routing.atfer` 装饰器封装接收消息处理函数。
- 该抽象类通常在对应版本中进行了继承，如：`ocpp.v16.ChargePoint`，添加了 `_call`，`_call_result`，`_ocpp_version` 属性值
...
|:---|:---|:---|
|id|str|充电设备 ID|
|connection|obj|[uwebsocket](https://python.quectel.com/doc/API_reference/zh/networklib/uwebsocket.html "QuecPython uwebsocket 模块使用说明") 实例对象|
...
- [OCPP v1.6 请求消息数据结构](./请求与应答消息数据结构说明_V16.md#请求消息结构体)
...
- 该属性主要用于二次开发时，封装接口时方便使用
- [OCPP v1.6 应答消息数据结构](./请求与应答消息数据结构说明_V16.md#应答消息结构体)
...
class ChargePoint(cp):
    ...
    def on_cancel_reservation(self, reservation_id):
        ...
        # 当收到消息后，可以在此处进行业务功能的处理，或者先进行应答，然后再在 after 装饰的函数内进行业务处理。
...
- 使用时，从对应的版本中导入 `ChargePoint` 类，如：`from usr.ocpp.v16 import ChargePoint as cp`，该版本中的类的 `_call` 和 `_call_result` 方法分别继承了对应版本的 `ocpp.v16.call` 对象和 `ocpp.v16.call_result` 对象, 用于使用对应版本的请求消息结构体和应答消息结构体。

**注意：**

以下只分别列出了一个客户发送消息和一个接收消息的处理函数的示例做为参考，实际项目需要使用哪些请求消息和接收消息，需要根据项目的实际要求进行开发
